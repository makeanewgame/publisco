...
→ 86.5 (2026-09-07 — DİKKAT: bu artış DÖNÜŞTÜRÜCÜDE bir iyileşme DEĞİL, ÖLÇÜM ARACININ düzeltilmesi. `converter.py`'ye hiç dokunulmadı. `normal-text-novel_son-sans`'ın golden `expected_paragraph_range`'i 14200-17200 → 7800-11700 yapıldı: eski değer yanlış birimden türetilmişti (kitabın kendi metadata notu onu "29 blocks/page × 541 pages" diye açıklıyor — yani paragraf değil BLOK/SATIR sayılmış; kitapta gerçekten 16089 gövde satırı var). Gerçek paragraf sayısı ~9134, bizim çıktımız zaten 9177'ydi (%99.5 uyum) — yani üretimimiz baştan doğruydu, cetvel yanlıştı. Kitap 89.8→97.6, Structure %56→%100. Aynı birim hatası başka golden kitaplarda da olabilir ama kullanılan doğrulama yöntemi yalnızca TEK SÜTUNLU düz nesirde geçerli olduğundan (çok sütunlu kitaplarda diğer sütunları paragraf başı sayıyor) diğer 38 kitaba DOKUNULMADI — bkz. NOTES.md.)
→ 86.5 (2026-09-07: girintisiz İÇİNDEKİLER/DİZİN sayfalarının tek paragrafa yapışması düzeltildi — `_looks_like_toc_entry`, nokta liderleri + sayfa numarası imzasıyla. `book-with-images_966108` 78.7→79.1; genel skor değişmedi çünkü kazanç `structure`'ın paragraf-sayısı penceresinde küçük kalıyor ama OKUNABİLİRLİK kazancı büyük: o kitapta içindekiler+şekiller/çizelgeler dizini 4369 karakterlik TEK paragraf iken artık her giriş kendi paragrafında (sayfa 5: 1→35 paragraf, sayfa 6: 6→23, sayfa 8-11: 1:1). Regresyon yok; kural tüm külliyatta ölçülerek daraltıldı (369→177 satır, gövde metni ve OCR gürültüsü elendi). Detaylar TAMAMLANANLAR.md'de.)
→ **86.5 (2026-09-08: Symbol ailesi fontlardan gelen Private Use Area kod noktaları gerçek Unicode'a çevriliyor — `book-with-images_966108`'de Kindle'da kutu (□) görünen 281 karakter σ/τ/φ oldu, kalan PUA sıfır. Genel skor değişmedi; o kitap 79.1→78.8 DÜŞTÜ ama bu kalite kaybı değil ÖLÇÜM GÖRÜNÜRLÜĞÜ: fix 1:1 karakter değişimi olduğundan yapıyı değiştiremez, Yunan harfleri artık kelime sayıldığı için tekrar eden formül parçaları `duplicate_ngram_ratio`'da ilk kez görünüyor (0.1258→0.1304; tekrar bloğu 398'de sabit, kırık kelime iki durumda da 0 — ayrıca ölçüldü). Font adına gate şart: külliyatta 2384 Wingdings/Webdings PUA karakteri var, gate olmasa anlamsız Yunan harfine dönerlerdi. Suite 83→87. Detaylar TAMAMLANANLAR_v2.md'de.)
→ **87.1 (2026-09-08 — DİKKAT: bu artış DÖNÜŞTÜRÜCÜDE bir iyileşme DEĞİL, ÖLÇÜM ARACININ düzeltilmesi; `converter.py`'ye hiç dokunulmadı. NOTES.md'de açık duran bir TEŞHİS ÇÜRÜTÜLDÜ: üç kitaptaki skor düşüşü boş-sayfa-atlamaya + golden `expected_paragraph_range`'e bağlanıyordu, ama ölçünce cplus'ta yalnızca 4/530 boş sayfa olduğu ve `Structure`'ın zaten %100 olduğu görüldü — golden'ı çıktımıza yaklaştırmak gerçek sorunu örtecekti. Asıl sebep golden'daki ÜÇ YANLIŞ ALINTILANMIŞ `must_include_phrase` çıktı (parantezli ara motif atlanmış; cümle erken noktalanmış; ':' yerine '.' yazılmış) — üçünde de bizim çıktımız PDF'le birebir aynıydı. `technical-with-code_cplus-conherince` 81.3→91.0, `technical-with-code_functional-programing` 70.2→89.7 (fast subset'te değil). Ders: golden'ı 'kalibrasyon artığı' diye düzeltmeden önce hipotezi ölç. Detaylar TAMAMLANANLAR_v2.md'de.)**
